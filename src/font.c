...
void reset_dialogue_boxes()
{
	s32 i;
	s32 width = is_widescreen() ? 424 : 320;
	for (i = 0; i < DIALOGUEBOXBACKGROUND_COUNT; i++) {
        ...
        (*gDialogueBoxBackground)[i].x2 = width - 1;
        (*gDialogueBoxBackground)[i].y2 = SCREEN_HEIGHT - 1;
        (*gDialogueBoxBackground)[i].width = width;
...
void draw_text_stretchpos(Gfx **displayList, s32 xpos, s32 ypos, char *text, s32 alignmentFlags) {
	if(is_widescreen() && xpos != POS_CENTRED) {
		xpos = ((xpos*424.0f)/320)+0.5f;
	}
    draw_text(displayList, xpos, ypos, text, alignmentFlags);
}
