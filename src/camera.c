...
void func_80065EA0(void) {
	...
	//Widescreen aspect ratio adjustment
	if(is_widescreen()) {
		aspect *= 424.0f/320;
	}
    guPerspectiveF(D_80120EE0, &perspNorm[10], CAMERA_DEFAULT_FOV, aspect, CAMERA_NEAR, CAMERA_FAR, CAMERA_SCALE);
...
void update_camera_fov(f32 camFieldOfView) {
    ...
    if (CAMERA_MIN_FOV < camFieldOfView && camFieldOfView < CAMERA_MAX_FOV && camFieldOfView != gCurCamFOV) {
        ...
        guPerspectiveF(D_80120EE0, &perspNorm[10], camFieldOfView, aspect, CAMERA_NEAR, CAMERA_FAR, CAMERA_SCALE);
        func_8006F870(D_80120EE0, D_80120FE0);
    }
}
...
void resize_viewport_stretch(s32 viewPortIndex, s32 x1, s32 y1, s32 x2, s32 y2)
{
	if(is_widescreen()) {
		//Scale viewport coordinates to 424
		x1 = ((x1*424.0f)/320)+0.5f;
		x2 = ((x2*424.0f)/320)+0.5f;
	}
	func_80066940(viewPortIndex, x1, y1, x2, y2);
}
...
void set_ortho_zoom_flag(s32 flag)
{
	ortho_zoom_flag = flag;
}
...
void func_80067F2C(Gfx **dlist, s32 *arg1) {
	...
	//Do ortho scale adjustment
	if(is_widescreen() && !ortho_zoom_flag) {
		gOrthoMatrix[3][3] = 212.0f;
	} else {
		gOrthoMatrix[3][3] = 160.0f;
	}
    widthAndHeight = get_video_width_and_height_as_s32();
