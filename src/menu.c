...
// Widescreen Title screen cinematic text
unk800DF83C D_800DF83C_wide[10] = {
    { "TIMBER", 14.0f, 14.5f, 16.5f, 17.0f, -80.0f, 208.0f, 212.0f, 208.0f, 504.0f, 208.0f },
    { "BUMPER", 19.0f, 19.5f, 21.5f, 22.0f, -80.0f, 208.0f, 212.0f, 208.0f, 504.0f, 208.0f },
    { "CONKER", 24.0f, 24.5f, 26.5f, 27.0f, -80.0f, 208.0f, 212.0f, 208.0f, 504.0f, 208.0f },
    { "TIPTUP", 29.0f, 29.5f, 31.5f, 32.0f, -80.0f, 208.0f, 212.0f, 208.0f, 504.0f, 208.0f },
    { "DRUMSTICK", 35.0f, 35.5f, 37.5f, 38.0f, -80.0f, 208.0f, 212.0f, 208.0f, 504.0f, 208.0f },
    { "PIPSY", 40.0f, 40.5f, 42.5f, 43.0f, -80.0f, 208.0f, 212.0f, 208.0f, 504.0f, 208.0f },
    { "BANJO", 45.0f, 45.5f, 47.5f, 48.0f, -80.0f, 208.0f, 212.0f, 208.0f, 504.0f, 208.0f },
    { "KRUNCH", 50.0f, 50.5f, 52.5f, 53.0f, -80.0f, 208.0f, 212.0f, 208.0f, 504.0f, 208.0f },
    { "WIZPIG", 57.5f, 58.0f, 60.0f, 60.5f, -80.0f, 208.0f, 212.0f, 208.0f, 504.0f, 208.0f },
    { "DIDDY", 63.5f, 64.0f, 66.0f, 66.5f, -80.0f, 208.0f, 212.0f, 208.0f, 504.0f, 208.0f }
};
...
static char *wideon_strs[4] = { "WIDE-SCREEN ON", "WIDE-SCREEN OUI", "BREITBILD EIN", "" };
static char *wideoff_strs[4] = { "WIDE-SCREEN OFF", "WIDE-SCREEN NON", "BREITBILD AUS", "" };
...
void load_menu_text(s32 language) {
    ...
    if (gMenuText != NULL) {
		...
		gMenuText[182] = wideoff_strs[langIndex-1];
		gMenuText[183] = wideon_strs[langIndex-1];
...
static void set_menu_element_xpos(MenuElement *element, s16 left, s16 center, s16 right)
{
	element->left = left;
...
static void fix_pos_data()
{
	s32 screen_w = is_widescreen() ? 424 : 320;
	s32 i;
	//Setup caution menu positions
	set_menu_element_xpos(&gCautionMenuTextElements[0], (screen_w/2)+1, (screen_w/2)+1, (screen_w/2)+1);
	for(i=1; i<13; i++) {
		set_menu_element_xpos(&gCautionMenuTextElements[i], screen_w/2, screen_w/2, screen_w/2);
	}
	//Setup game select positions
	set_menu_element_xpos(&gGameSelectTextElemsNoAdv2[0], (screen_w/2)+1, (screen_w/2)+1, (screen_w/2)+1);
	set_menu_element_xpos(&gGameSelectTextElemsNoAdv2[1], screen_w/2, screen_w/2, screen_w/2);
	set_menu_element_xpos(&gGameSelectTextElemsNoAdv2[3], screen_w/2, screen_w/2, screen_w/2);
	set_menu_element_xpos(&gGameSelectTextElemsNoAdv2[5], screen_w/2, screen_w/2, screen_w/2);
	set_menu_element_xpos(&gGameSelectTextElemsWithAdv2[0], (screen_w/2)+1, (screen_w/2)+1, (screen_w/2)+1);
	set_menu_element_xpos(&gGameSelectTextElemsWithAdv2[1], screen_w/2, screen_w/2, screen_w/2);
	set_menu_element_xpos(&gGameSelectTextElemsWithAdv2[3], screen_w/2, screen_w/2, screen_w/2);
	set_menu_element_xpos(&gGameSelectTextElemsWithAdv2[5], screen_w/2, screen_w/2, screen_w/2);
	set_menu_element_xpos(&gGameSelectTextElemsWithAdv2[7], screen_w/2, screen_w/2, screen_w/2);
	//Setup record positions
	set_menu_element_xpos(&D_800E0BEC[0], ((screen_w*3)/2)-128, (screen_w/2)-128, -((screen_w/2)-128));
	set_menu_element_xpos(&D_800E0BEC[1], ((screen_w*3)/2)-28, (screen_w/2)-28, -((screen_w/2)-28));
	for(i=3; i<6; i++) {
		set_menu_element_xpos(&D_800E0BEC[i], ((screen_w*3)/2)-28, (screen_w/2)-28, -((screen_w/2)-28));
	}
	set_menu_element_xpos(&D_800E0BEC[2], ((screen_w*3)/2)+80, (screen_w/2)+80, -((screen_w/2)+80));
	set_menu_element_xpos(&D_800E0BEC[6], ((screen_w*3)/2)+80, (screen_w/2)+80, -((screen_w/2)+80));
	//Setup player icon finish positions
	for(i=0; i<8; i++) {
		s32 x = ((screen_w/2)+95)-(i*33);
		set_menu_element_xpos(&D_800E0CEC[i], x+screen_w, x, x-screen_w);
	}
	//Setup finish text positions
	set_menu_element_xpos(&D_800E0CEC[8], ((screen_w*3)/2)+1, (screen_w/2)+1, -((screen_w/2)+1));
	set_menu_element_xpos(&D_800E0CEC[9], ((screen_w*3)/2)-1, (screen_w/2)-1, -((screen_w/2)-1));
	//Setup record positions
	set_menu_element_xpos(&D_800E0E4C[0], ((screen_w*3)/2)+1, (screen_w/2)+1, -((screen_w/2)+1));
	set_menu_element_xpos(&D_800E0E4C[1], ((screen_w*3)/2)-1, (screen_w/2)-1, -((screen_w/2)-1));
	for(i=0; i<2; i++) {
		set_menu_element_xpos(&D_800E0E4C[(i*3)+2], ((screen_w*3)/2)-112, (screen_w/2)-112, -((screen_w/2)-112));
		set_menu_element_xpos(&D_800E0E4C[(i*3)+3], ((screen_w*3)/2)+26, (screen_w/2)+26, -((screen_w/2)+26));
		set_menu_element_xpos(&D_800E0E4C[(i*3)+4], ((screen_w*3)/2)+98, (screen_w/2)+98, -((screen_w/2)+98));
	}
	//Setup player icon positions
	for(i=0; i<4; i++) {
		set_menu_element_xpos(&D_800E1048[i][2], (screen_w/2)-96, (screen_w/2)-96, (screen_w/2)-96);
		D_800E13A8[i*3] = (screen_w/2)-96;
		set_menu_element_xpos(&D_800E1048[i+4][2], (screen_w/2)+60, (screen_w/2)+60, (screen_w/2)+60);
		D_800E13A8[(i*3)+12] = (screen_w/2)+60;
		set_menu_element_xpos(&D_800E1048[i+1][0], (screen_w/2)-128, (screen_w/2)-128, (screen_w/2)-128);
		D_800E13A8[(i*3)+1] = (screen_w/2)-128;
		set_menu_element_xpos(&D_800E1048[i+5][0], (screen_w/2)+28, (screen_w/2)+28, (screen_w/2)+28);
		D_800E13A8[(i*3)+13] = (screen_w/2)+28;
		set_menu_element_xpos(&D_800E1048[i+1][1], (screen_w/2)-30, (screen_w/2)-30, (screen_w/2)-30);
		D_800E13A8[(i*3)+2] = (screen_w/2)-30;
		set_menu_element_xpos(&D_800E1048[i+5][1], (screen_w/2)+126, (screen_w/2)+126, (screen_w/2)+126);
		D_800E13A8[(i*3)+14] = (screen_w/2)+126;
	}
	//Setup multiplayer icon positions
	for(i=0; i<3; i++) {
		D_800E13A8[(i*3)+72] = D_800E13A8[i*3];
		D_800E13A8[(i*3)+73] = D_800E13A8[(i*3)+1];
		D_800E13A8[(i*3)+74] = D_800E13A8[(i*3)+2];
		D_800E13A8[(i*3)+81] = D_800E13A8[(i*3)+12];
		D_800E13A8[(i*3)+82] = D_800E13A8[(i*3)+13];
		D_800E13A8[(i*3)+83] = D_800E13A8[(i*3)+14];
	}
	//Setup multiplayer icon positions
	for(i=0; i<4; i++) {
		D_800E13A8[(i*3)+126] = (screen_w/2)-20;
		D_800E13A8[(i*3)+127] = (screen_w/2)-52;
		D_800E13A8[(i*3)+128] = (screen_w/2)+46;
	}
	//Setup end position 
	set_menu_element_xpos(&D_800E1048[0][0], ((screen_w*3)/2)+1, (screen_w/2)+1, -((screen_w/2)+1));
	set_menu_element_xpos(&D_800E1048[0][1], (screen_w*3)/2, screen_w/2, -screen_w/2);
	//Setup credits positions
	for(i=0; i<8; i++) {
		set_menu_element_xpos(&D_800E1B50[i], (screen_w*3)/2, screen_w/2, -screen_w/2);
	}
	//Setup file positions
	for(i=0; i<3; i++) {
		D_800E03CC[i].unk0 = (i*92)+((screen_w/2)-136);
	}
	//Setup controller pak positions
	set_menu_element_xpos(&D_800DFCB4[0], (screen_w/2)+1, (screen_w/2)+1, (screen_w/2)+1);
	for(i=1; i<6; i++) {
		set_menu_element_xpos(&D_800DFCB4[i], screen_w/2, screen_w/2, screen_w/2);
	}
}
...
void draw_menu_elements(s32 arg0, MenuElement *elem, f32 arg2) {
	s32 half_screen_w = is_widescreen() ? 212 : 160;
    s32 s5;
    s32 tempX, tempY;
    s32 xPos, yPos;
            ...
            if (&D_80126850 != elem->unk14_a.element) {
                ...
                switch (elem->elementType) {
                    ...
                    case 5:
                        ...
                        func_80068508(1);
                        func_8007BF1C(0);
                        gMenuImageStack[elem->unk14_a.value].unkC = xPos - half_screen_w;
...
void menu_logos_screen_init(void) {
    ...
    if (osTvType == TV_TYPE_PAL) {
        func_80066940(0, 0, 38, screen_w, SCREEN_HEIGHT - 16);
        set_viewport_properties(0, VIEWPORT_AUTO, VIEWPORT_AUTO, screen_w, SCREEN_HEIGHT + 44);
    } else {
        func_80066940(0, 0, 40, screen_w, SCREEN_HEIGHT - 44);
        set_viewport_properties(0, VIEWPORT_AUTO, VIEWPORT_AUTO, screen_w, SCREEN_HEIGHT);
    }
    func_80066610();
    func_80066818(0, 1);
}
...
s32 menu_logo_screen_loop(s32 updateRate) {
    ...
    s32 yOffset;
    s32 yOffsetShadow;
	u32 half_screen_w = is_widescreen() ? 212 : 160;
...
// This uses doubles instead of floats for precision in the counting.
    if (sBootScreenTimer < 8.5) {
        ...
        if (opacity != 0) {
            ...
            draw_text(&D_801263A0, half_screen_w-1, yOffset + 212, gRareCopyrightString, 0xC);
            draw_text(&D_801263A0, half_screen_w+1, yOffset + 212, gRareCopyrightString, 0xC);
...
void func_800841B8(s32 arg0) {
    ...
    draw_text(&D_801263A0, half_screen_w+1, 0x23, gMenuText[36], 0xC);
    set_text_color(255, 255, 255, 0, 0xFF);
    draw_text(&D_801263A0, half_screen_w, 0x20, gMenuText[36], 0xC);
...
s32 menu_boot_loop(s32 arg0) {
    ...
    if (y < 300) {
		s32 half_screen_w = is_widescreen() ? 212 : 160;
        render_textured_rectangle(&D_801263A0, &sGameTitleTileOffsets, half_screen_w, y, 255, 255, 255, 255);
        func_8007B3D0(&D_801263A0);
    }
...
void menu_magic_codes_init(void) {
    ...
    D_801263E0 = 0;
    func_800C01D8(&sMenuTransitionFadeOut);
    set_current_dialogue_box_coords(7, half_screen_w-110, 0x32, half_screen_w+110, 0x84);
...
void render_magic_codes_ui(s32 arg0) {
    ...
    set_text_font(FONT_LARGE);
    set_text_color(0, 0, 0, 0xFF, 0x80);
    draw_text(&D_801263A0, half_screen_w+1, 0x23, gMenuText[17], 0xC); // gMenuText[17] = "MAGIC CODES"
    set_text_color(0xFF, 0xFF, 0xFF, 0, 0xFF);
    draw_text(&D_801263A0, half_screen_w, 0x20, gMenuText[17], 0xC); // gMenuText[17] = "MAGIC CODES"
    set_text_font(FONT_COLOURFUL);
    set_text_color(0xFF, 0xFF, 0xFF, 0, 0xFF);
    ...
    if (D_801263E0 != 0) {
        ...
        set_current_dialogue_box_coords(6, half_screen_w-84, yPos - 0x1C, half_screen_w+84, yPos + 0x1C);
...
void render_magic_codes_list_menu_text(s32 arg0) {
    ...
    set_text_font(FONT_LARGE);
    set_text_color(0, 0, 0, 0xFF, 0x80);
    draw_text(&D_801263A0, half_screen_w+1, 0x23, gMenuText[20], 0xC); // MAGIC CODES LIST
    set_text_color(0xFF, 0xFF, 0xFF, 0, 0xFF);
    draw_text(&D_801263A0, half_screen_w, 0x20, gMenuText[20], 0xC); // MAGIC CODES LIST
    ...
    for (i = D_801263E0; (i < D_801263E0 + D_80126C70) && (i < numOfUnlockedCheats); i++) {
        ...
        draw_text(&D_801263A0, half_screen_w-112, s2, (char *)(*gCheatsAssetData) + ((*gCheatsAssetData + 1)[D_80126C80[i] * 2 + 1]), 0);
        if ((1 << D_80126C80[i]) & gActiveMagicCodes) {
            draw_text(&D_801263A0, half_screen_w+96, s2, gMenuText[21], 0); // ON
        } else {
            draw_text(&D_801263A0, half_screen_w+96, s2, gMenuText[22], 0); // OFF
        }
        if (i == gOptionsMenuItemIndex) {
            set_text_color(0xFF, 0xFF, 0xFF, 0, 0xFF);
        }
    ...
    if (D_801263BC & 8) {
        render_textured_rectangle(&D_801263A0, &D_800E043C, half_screen_w, s2 + 8, 0xFF, 0xFF, 0xFF, 0xFF);
    }
}
#else
...
void draw_character_select_text(s32 arg0) {
    ...
    if (gMenuDelay >= -0x16 && gMenuDelay < 0x17) {
        ...
        draw_text(&D_801263A0, half_screen_w+1, 0x23, gMenuText[135], 0xC);
        set_text_color(255, 255, 255, 0, 0xFF);
        // Draw "Player Select" text
        draw_text(&D_801263A0, half_screen_w, 0x20, gMenuText[135], 0xC);
        if (gNumberOfReadyPlayers == gNumberOfActivePlayers && gNumberOfActivePlayers > 0) {
            yPos = 0xD0;
            if (osTvType == TV_TYPE_PAL) {
                yPos = 0xEA;
            }
            draw_text(&D_801263A0, half_screen_w, yPos, D_800E8230 /* "OK?" */, 0xC);
        }
        func_8007B3D0(&D_801263A0);
...
// This first section is the shadow
    ...
    gMenuImageStack[imageID].unkC = xOffset - (half_screen_w-1);
...
void render_file_select_menu(UNUSED s32 updateRate) {
    ...
    for (i = 0; i < 3; i++) { // 3 files
        ...
        func_80080580(0, D_800E03CC[i].unk0 - half_screen_w, 0x78 - D_800E03CC[i].unk2, D_800E03CC[i].unk4,
    ...
    draw_text(&D_801263A0, half_screen_w+1, 0x13, gMenuText[76], 4);
    set_text_color(255, 255, 255, 0, 255);
    draw_text(&D_801263A0, half_screen_w, 0x10, gMenuText[76], 4);
    set_text_color(255, 255, 255, 0, 255);
    y += 0xBB;
    if (D_80126484 != 0) {
        if (D_80126494 == 0) {
            set_text_font(FONT_COLOURFUL);
            draw_text(&D_801263A0, half_screen_w, y, gMenuText[77], 0xC);
        } else if (D_80126494 == 1) {
            set_text_font(FONT_COLOURFUL);
            draw_text(&D_801263A0, half_screen_w, y, gMenuText[78], 0xC);
        } else {
            draw_text(&D_801263A0, half_screen_w, y, &D_800E8234, 0xC);
        }
        return;
    }
    if (D_80126488 != 0) {
        if (D_80126494 == 0) {
            set_text_font(FONT_COLOURFUL);
            draw_text(&D_801263A0, half_screen_w, y, gMenuText[79], 0xC);
        } else {
            draw_text(&D_801263A0, half_screen_w, y, &D_800E8238, 0xC);
        }
        return;
    }
    if (D_80126CC0 == 0) {
        ...
        draw_text(&D_801263A0, half_screen_w-70, y, gMenuText[80], 0xC);
        if (D_801263E0 == 2) {
            set_text_color(255, 255, 255, phi_v0_2, 255);
        } else {
            set_text_color(255, 255, 255, 0, 255);
        }
        draw_text(&D_801263A0, half_screen_w+70, y, gMenuText[81], 0xC);
        return;
    }
}
#else
...
void render_track_select_setup_ui(s32 arg0) {
    ...
    if ((gMenuDelay >= -0x16) && (gMenuDelay < 0x1F)) {
        ...
        if ((D_801263E0 == -1) || ((D_801263E0 == 2) && (D_801269C8 == 4) && is_adventure_two_unlocked())) {
            ...
            for (i = 0; i < 2; i++) {
                for (j = 0; j < 4; j += 2) {
                    ...
                    draw_text(&D_801263A0, (half_screen_w+1) - j, s7 - j, gMenuText[146 + i], 0xC);
                }
                s7 += 16;
            }
        }
        if (D_801269C8 < 4) {
            ...
            draw_text(&D_801263A0, (half_screen_w-104), 0x48 + sp80, gMenuText[9], 8);  // gMenuText[9] = "BEST TIME"
            draw_text(&D_801263A0, (half_screen_w-104), 0x5C + sp80, gMenuText[10], 8); // gMenuText[10] = "BEST LAP"
            ...
            draw_text(&D_801263A0, (half_screen_w+90), 0x48 + sp80, &sp78, 0xC);
            decompress_filename_string(settings->flapInitialsPtr[gPlayerSelectVehicle[0]][gTrackIdForPreview], &sp78, 3);
            draw_text(&D_801263A0, (half_screen_w+90), 0x5C + sp80, &sp78, 0xC);
            ...
            if (D_801263E0 != -1) {
                if (gNumberOfActivePlayers == 1) {
                    // Glow effect
                    set_current_dialogue_box_coords(7, half_screen_w-26, sp80 + 0x70, half_screen_w+26, sp80 + 0x89);
                    render_dialogue_box(&D_801263A0, 0, 0, 7);
                    if (D_801263E0 <= 0) {
                        render_textured_rectangle(&D_801263A0, &D_800E05B4, half_screen_w-24, sp80 + 0x72, 0xFF, 0xFF, 0xFF, sMenuGuiOpacity);
                    } else {
                        render_textured_rectangle(&D_801263A0, &D_800E05C4, half_screen_w-24, sp80 + 0x72, 0xFF, 0xFF, 0xFF, sMenuGuiOpacity);
                        for (i = 0; i < 2; i++) {
                            s32 yTemp = 0x97 + sp80 + (i * 0x18);
                            if (i == D_800E0414) {
                                render_textured_rectangle(&D_801263A0, D_800E0648[(i * 3) + 1], half_screen_w-56, yTemp, 0xFF, 0xFF, 0xFF, sMenuGuiOpacity);
                            } else {
                                render_textured_rectangle(&D_801263A0, D_800E0648[(i * 3) + 2], half_screen_w-56, yTemp, 0xFF, 0xFF, 0xFF, sMenuGuiOpacity);
                            }
                        }
                    }
                } else {
                    if (!sp74) {
                        ...
                        for (i = 0; i < gNumberOfActivePlayers; i++) {
                            ...
                            if (((gNumberOfActivePlayers == 1) && (i == D_801263E0)) || ((gNumberOfActivePlayers >= 2) && (D_801269C4[i] == 0))) {
                                s32 temp_v0_9 = D_800E0688[s3 + (i * 2)] + (half_screen_w-160);
                            ...
                            // "Player" text image
                            render_textured_rectangle(&D_801263A0, D_800E0660[i], D_800E0688[s3 + (i * 2)] + (half_screen_w-160), D_800E0688[s3 + (i * 2) + 1] + sp80, 0xFF, 0xFF, 0xFF, sMenuGuiOpacity);
                        }
                    }
                }
                                ...
                                if (gNumberOfActivePlayers > 0) {
                                    for (j = 0; j < gNumberOfActivePlayers; j++) {
                                        if (i == gPlayerSelectVehicle[j]) {
                                            // Highlighted
                                            render_textured_rectangle(&D_801263A0, D_800E0624[(i * 3) + 1], D_800E06B0[s3 + j] + (half_screen_w-160), yTemp, 0xFF, 0xFF, 0xFF, sMenuGuiOpacity);
                                        } else if (settings->courseFlagsPtr[gTrackIdForPreview] & 2) {
                                            // Not highlighted
                                            render_textured_rectangle(&D_801263A0, D_800E0624[(i * 3) + 2], D_800E06B0[s3 + j] + (half_screen_w-160), yTemp, 0xFF, 0xFF, 0xFF, sMenuGuiOpacity);
                                        } else {
                                            // Not available (Ghosted out)
                                            render_textured_rectangle(&D_801263A0, D_800E0624[(i * 3) + 2], D_800E06B0[s3 + j] + (half_screen_w-160), yTemp, 0xFF, 0xFF, 0xFF, sMenuGuiOpacity / 2);
                                        }
                                    }
                                }
                                yTemp += 0x18;
                ...
                if (gNumberOfActivePlayers == 1) {
                    if (D_801263E0 == 0) {
                        ...
                        render_textured_rectangle(&D_801263A0, D_800E0624[gPlayerSelectVehicle[0] * 3], half_screen_w-11, s7, 0xFF, 0xFF, 0xFF, sMenuGuiOpacity);
                    } else {
                        // Draw T.T. image for one player
                        render_textured_rectangle(&D_801263A0, D_800E0648[D_800E0414 * 3], half_screen_w-11, sp80, 0xFF, 0xFF, 0xFF, sMenuGuiOpacity);
                    }
                }
                if ((gNumberOfActivePlayers == 2) && (!sp74)) {
                    s7 = sp80;
                    // Draw vehicle image for first player
                    render_textured_rectangle(&D_801263A0, D_800E0624[gPlayerSelectVehicle[0] * 3], half_screen_w-81, s7, 0xFF, 0xFF, 0xFF, sMenuGuiOpacity);
                    if (gPlayerSelectVehicle[1] == 2) {
                        s7 = sp80 + 2;
                    }
                    // Draw vehicle image for second player
                    render_textured_rectangle(&D_801263A0, D_800E0624[gPlayerSelectVehicle[1] * 3], half_screen_w+16, s7, 0xFF, 0xFF, 0xFF, sMenuGuiOpacity);
                }
                ...
                if (sp74) {
                    ...
                    set_text_font(FONT_COLOURFUL);
                    set_text_color(0, 0, 0, 0xFF, 0x80);
                    draw_text(&D_801263A0, (half_screen_w-160) + D_800E06E4[6] + D_800E06E4[0] + 1, D_800E06E4[7] + D_800E06E4[1] + 1, gMenuText[136], 0xC);
                    set_text_color(0xFF, 0xFF, 0xFF, 0, 0xFF);
                    draw_text(&D_801263A0, (half_screen_w-160) + D_800E06E4[6] + D_800E06E4[0] - 1, D_800E06E4[7] + D_800E06E4[1] - 1, gMenuText[136], 0xC);
        ...
        if (D_801269C8 != 5) {
            if (((D_801263E0 == 2) && !sp74) || ((D_801263E0 == 3) && sp74) || (D_801263E0 == 4)) {
                ...
                draw_text(&D_801263A0, half_screen_w, 0xAC, D_800E823C, 0xC); // D_800E823C = "OK?"
            }
        }
        sMenuGuiOpacity = 0xFF;
    }
...
void func_800983C0(s32 arg0) {
    ...
    set_text_font(FONT_LARGE);
    set_text_color(0, 0, 0, 0xFF, 0x80);
    draw_text(&D_801263A0, half_screen_w+1, 0x23, sp28, 0xC);
    draw_text(&D_801263A0, half_screen_w+1, 0x43, gMenuText[70], 0xC);
    set_text_color(255, 255, 255, 0, 0xFF);
    draw_text(&D_801263A0, half_screen_w, 0x20, sp28, 0xC);
    draw_text(&D_801263A0, half_screen_w, 0x40, gMenuText[70], 0xC);
    draw_text(&D_801263A0, half_screen_w, sp2C + 0xB0, gMenuText[138 + D_800E0FEC], 0xC);
    draw_text(&D_801263A0, half_screen_w, sp2C + 0xD0, sp24, 0xC);
}
...
void menu_credits_init(void) {
    ...
    set_background_fill_colour(0, 0, 0);
    if (osTvType == TV_TYPE_PAL) {
        func_80066940(0, 0, 38, screen_w, 224);
        set_viewport_properties(0, VIEWPORT_AUTO, VIEWPORT_AUTO, screen_w, SCREEN_HEIGHT + 44);
    } else {
        func_80066940(0, 0, 40, screen_w, 196);
        set_viewport_properties(0, VIEWPORT_AUTO, VIEWPORT_AUTO, screen_w, SCREEN_HEIGHT);
    }
    func_80066610();
    func_80066818(0, 1);
...
void draw_stretched_frame(s32 arg0)
{
	f32 old_yscale = D_800DF454;
	f32 old_scale = gMenuImageStack[arg0].unk8;
	f32 old_x = gMenuImageStack[arg0].unkC;
	if(is_widescreen()) {
		D_800DF454 *= 320.0f/424;
		gMenuImageStack[arg0].unk8 *= 424.0f/320;
		gMenuImageStack[arg0].unkC *= 424.0f/320;
	}
	func_8009CA60(arg0);
	gMenuImageStack[arg0].unkC = old_x;
	gMenuImageStack[arg0].unk8 = old_scale;
	D_800DF454 = old_yscale;
}
